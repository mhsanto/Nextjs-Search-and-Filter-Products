...
const Product: React.FC<ProductProps> = ({ item }) => {
  ...
  //   }, 3000);
  // });
  // const items: any = await awaitItems;
  return (
 
      <Link
        key={item.id}
        href={`${item.id}`}
        className=" max-w-fit flex-grow  bg-white text-black rounded-sm px-2 group overflow-hidden transition-all duration-300 cursor-pointer hover:ring-2 border-transparent"
      >
