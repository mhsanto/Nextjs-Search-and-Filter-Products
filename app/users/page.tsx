                ...
                {sortedPeople.map(person=>{
                    return (
                        <tr key={person.email} className="bg-white">
                            ...
                            <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">{person.title}</td>
                            <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">{person.email}</td>
                            <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">{person.role}</td>
                        </tr>
                    )
                })}
...
const SortableComponent: React.FC<SortableComponentProps> = ({
  ...
  return (
    <th
      className="py-3.5 pl-4 pr-3 text-left text-sm text-gray-900 sm:pl-5"
      scope="col"
    >
      <button
      ...
      >
        ...
        <span
          className={`${
            acceding === "name"
              ? "text-gray-900 bg-gray-200 group-hover:bg-gray-400"
              : "invisible text-gray-400 group-hover:visible "
          } flex-none ml-2 rounded`}
        >
          <ArrowDown className={`${descending ? "-rotate-180" : ""}`} />
        </span>{" "}
      </button>
    </th>
  );
